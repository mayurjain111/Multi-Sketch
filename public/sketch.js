...
function mouseDragged() {
  ...
  cursorX = 0.9 * cursorX + 0.1 * mousex;
  cursorY = 0.9 * cursorY + 0.1 * mousey;
  var data = {
    x: cursorX,
    y: cursorY
  };
  ...
  } else fill(lineColor[0], lineColor[1], lineColor[2]);
  ellipse(cursorX, cursorY, lineThickness, lineThickness);
}

function draw() {
  document.getElementById('brush-color').style.backgroundColor =
    'rgb(' + lineColor[0] + ',' + lineColor[1] + ',' + lineColor[2] + ')';
  if (mouseIsPressed) {
    mouseDragged();
  }
}
